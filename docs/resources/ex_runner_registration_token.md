...
- `keepers` (Map of String) <i style="color:red;font-weight: bold">(ForceNew)</i> Arbitrary map of values that, when changed, forces a new registration token to be generated. Use it to trigger rotation (the SemaphoreUI API exposes no other way to rotate a token in place). Adopt the first imported keepers map without rotation; subsequent changes require replacement.
