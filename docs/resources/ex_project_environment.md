...
- `environment_json` (String) Lossless JSON representation of environment. Use jsonencode() for typed values; mutually exclusive with environment. Omission preserves existing values; configure jsonencode({}) to clear them. A JSON object; environment values must be scalar. Ensure that if an attribute is set, these are not set: "[environment]".
...
- `variables_json` (String) Lossless JSON representation of variables. Use jsonencode() for typed values; mutually exclusive with variables. Omission preserves existing values; configure jsonencode({}) to clear them. A JSON object; environment values must be scalar. Ensure that if an attribute is set, these are not set: "[variables]".
